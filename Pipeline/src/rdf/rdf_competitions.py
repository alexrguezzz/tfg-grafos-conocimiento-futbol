...
if __name__ == "__main__":
    ...
    parse_no_args("Genera RDF para competiciones canonicas.")
...
from src.utils.namespaces import EX, CLASS, PROP, RESOURCE, COMPETITION, NAME, ID_UNDERSTAT, competition_uri


REQUIRED_COLUMNS = ["id_competition", "name", "idUnderstat"]


def build_input_path() -> Path:
    return PROJECT_ROOT / "data" / "processed" / "canonical" / "competitions.csv"


def build_output_path() -> Path:
    output_dir = PROJECT_ROOT / "data" / "ttl"
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir / "competitions.ttl"
...
def build_graph(df: pd.DataFrame) -> Graph:
    if not all(col in df.columns for col in REQUIRED_COLUMNS):
        raise ValueError("Faltan columnas obligatorias en competitions.csv")
    ...
    for _, row in df.iterrows():
        ...
        node = competition_uri(competition_id)
        graph.add((node, RDF.type, COMPETITION))
        graph.add((node, RDFS.label, Literal(name or competition_id, datatype=XSD.string)))
...
def main() -> None:
    ...
    print("Serializando competiciones canonicas a RDF...")
    df = pd.read_csv(input_path, dtype={"idUnderstat": "string"})
