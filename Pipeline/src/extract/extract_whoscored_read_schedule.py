...
def build_output_path(leagues: list[str], seasons: list[str]):
    return build_raw_output_path("whoscored", "schedule", "read_schedule", leagues, seasons)


def main() -> None:
    args = parse_leagues_seasons_args("Extrae calendarios de WhoScored para una o varias ligas y temporadas.")
    import soccerdata as sd

    leagues = args.leagues
    seasons = args.seasons
    print(f"\nLeyendo calendario de partidos desde WhoScored: {', '.join(leagues)} / {', '.join(seasons)}...\n")
    whoscored = sd.WhoScored(leagues=leagues, seasons=seasons)
    df = whoscored.read_schedule().reset_index()
