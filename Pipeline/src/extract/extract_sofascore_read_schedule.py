...
def build_output_path(leagues: list[str], seasons: list[str]):
    return build_raw_output_path("sofascore", "schedule", "read_schedule", leagues, seasons)


def main() -> None:
    args = parse_leagues_seasons_args("Extrae calendarios de SofaScore para una o varias ligas y temporadas.")
    import soccerdata as sd

    leagues = args.leagues
    seasons = args.seasons
    print(f"\nLeyendo calendario de partidos desde SofaScore: {', '.join(leagues)} / {', '.join(seasons)}...\n")
    sofascore = sd.Sofascore(leagues=leagues, seasons=seasons)
    df = sofascore.read_schedule().reset_index()
