...
def build_output_path(leagues: list[str], seasons: list[str]):
    return build_raw_output_path("espn", "matchsheet", "read_matchsheet", leagues, seasons)


def main() -> None:
    args = parse_leagues_seasons_args("Extrae actas de partido de ESPN para una o varias ligas y temporadas.")
    import soccerdata as sd

    leagues = args.leagues
    seasons = args.seasons
    print(f"\nLeyendo actas de partido desde ESPN: {', '.join(leagues)} / {', '.join(seasons)}...\n")
    espn = sd.ESPN(leagues=leagues, seasons=seasons)
    df = espn.read_matchsheet().reset_index()
