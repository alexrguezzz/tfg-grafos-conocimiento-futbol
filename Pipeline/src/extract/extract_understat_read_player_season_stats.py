...
def build_output_path(leagues: list[str], seasons: list[str]):
    return build_raw_output_path("understat", "player_season_stats", "read_player_season_stats", leagues, seasons)


def main() -> None:
    args = parse_leagues_seasons_args("Extrae estadisticas de jugador por temporada de Understat para una o varias ligas y temporadas.")
    import soccerdata as sd

    leagues = args.leagues
    seasons = args.seasons
    print(f"\nLeyendo estadisticas de jugador por temporada desde Understat: {', '.join(leagues)} / {', '.join(seasons)}...\n")
